package com.shinobitac.proyectodemona.slideout;
...
public class SlideoutHelper {
	...
	// creamos una variable que denota de que actividad venimos al abrir el
	// menuslider
	private static Activity fromActivity;

	public static void prepare(Activity activity, int id, int width) {

		// este codigo de recyclar causa un bug al momento de regresar las
		// ventanas por querer reciclar elementos que ya fueron reciclados
		/*
		 * if (sCoverBitmap != null) { sCoverBitmap.recycle(); }
		 */
		// le asignamos la actividad de donde venimos a la variable global
	...
	// metodo creado por shinobitac para cuando de clic en algun menu del
	// menuslider
	// cierre la actividad donde este y no tenga recursividad
	public void closeActivityActual() {
		fromActivity.finish();
	}
